...
public extension Array where Element == Certificate {
    
    static var all: [Certificate] {
        get async throws {
            try await AppStoreConnect.v1
                .certificates.get()
                .value.data
...
public extension Array where Element == Certificate {
    ...
    static func certificates(ofType type: CertificateType...) async throws -> [Certificate] {
        try await AppStoreConnect.v1
...
public extension Array where Element == Certificate {
    ...
    static func certificates(forID id: Profile.ID) async throws -> [Certificate] {
        try await AppStoreConnect.v1
            .profiles.id(id)
            .certificates.get()
            .value.data
    ...
    static func certificates(forProfile profile: Profile) async throws -> [Certificate] {
        try await certificates(forID: profile.id)
    }
    
    static func certificates(forName name: Profile.Name) async throws -> [Certificate] {
