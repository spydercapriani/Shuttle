// Generated by Create API
// https://github.com/CreateAPI/CreateAPI

import Foundation
import Get
import URLQueryEncoder

extension AppStoreConnect.V1.AppStoreVersionPhasedReleases {
	public func id(_ id: String) -> WithID {
		WithID(path: "\(path)/\(id)")
	}

	public struct WithID {
		/// Path: `/v1/appStoreVersionPhasedReleases/{id}`
		public let path: String

		public func patch(_ body: Shuttle.AppStoreVersionPhasedReleaseUpdateRequest) -> Request<Shuttle.AppStoreVersionPhasedReleaseResponse> {
			Request(method: "PATCH", url: path, body: body, id: "appStoreVersionPhasedReleases-update_instance")
		}

		public var delete: Request<Void> {
			Request(method: "DELETE", url: path, id: "appStoreVersionPhasedReleases-delete_instance")
		}
	}
}
